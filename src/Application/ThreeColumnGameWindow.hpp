...
struct ThreeColumnGameWindow {
    static constexpr auto kBaseWindowId = "ThreeColumnGameWindow";

    observer_ptr<LeftSidebar> left_sidebar;
    observer_ptr<MainContent> main_content;
    observer_ptr<RightSidebar> right_sidebar;

    std::function<void()> on_toggle_menu_bar;

    ThreeColumnGameWindow(observer_ptr<LeftSidebar> lhs, observer_ptr<MainContent> main, observer_ptr<RightSidebar> rhs)
    ...
    void Render() const {
        ...
        if (ImGui::Begin("##LayoutWindow", nullptr, windowFlags)) {
            if (ImGui::BeginPopup("LayoutContextPopup")) {
                if (ImGui::MenuItem("Toggle Main Menu Bar")) {
                    if (on_toggle_menu_bar) {
                        on_toggle_menu_bar();
                    }
                }
                ImGui::EndPopup();
            }
            ImGui::OpenPopupOnItemClick("LayoutContextPopup", ImGuiPopupFlags_MouseButtonRight);

            constexpr ImGuiTableFlags tableFlags = ImGuiTableFlags_SizingStretchSame |
                                                   ImGuiTableFlags_NoHostExtendY |
                                                   ImGuiTableFlags_NoBordersInBody;

            if (ImGui::BeginTable(kBaseWindowId, 3, tableFlags, ImGui::GetContentRegionAvail())) {
